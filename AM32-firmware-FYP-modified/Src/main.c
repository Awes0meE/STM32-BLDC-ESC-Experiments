...
// #define FIXED_DUTY_MODE_POWER 10      //
...
// #define FIXED_SPEED_MODE_RPM  8000  //
// #define FIXED_SPEED_MODE_RPM_2  10000  //
// #define FIXED_SPEED_MODE_STAGE_TIME_S 10U
...
uint32_t fixed_speed_mode_counter = 0;
uint8_t fixed_speed_mode_stage = 0;
#endif
...
#ifdef FIXED_SPEED_MODE
    // Fixed-speed mode runs a simple staged RPM profile for bench testing.
    if (armed) {
        uint32_t fixed_speed_stage_ticks = LOOP_FREQUENCY_HZ * FIXED_SPEED_MODE_STAGE_TIME_S;
        uint32_t pole_pairs = eepromBuffer.motor_poles / 2;

        if (pole_pairs > 0) {
            if (fixed_speed_mode_stage == 0) {
                newinput = 48;
                adjusted_input = 48;
                target_e_com_time = 60000000UL / FIXED_SPEED_MODE_RPM / pole_pairs;
                fixed_speed_mode_counter++;

                if (fixed_speed_mode_counter >= fixed_speed_stage_ticks) {
                    fixed_speed_mode_stage = 1;
                    fixed_speed_mode_counter = 0;
                    speedPid.error = 0;
                    speedPid.integral = 0;
                    input_override = 48 * 10000;
                    target_e_com_time = 60000000UL / FIXED_SPEED_MODE_RPM_2 / pole_pairs;
                }
            } else if (fixed_speed_mode_stage == 1) {
                newinput = 48;
                adjusted_input = 48;
                target_e_com_time = 60000000UL / FIXED_SPEED_MODE_RPM_2 / pole_pairs;
                fixed_speed_mode_counter++;
...
int main(void)
{
    ...
    // 无 EEPROM 或空白 Flash 时，先显式钉死几项与桥臂驱动直接相关的安全位，
    // 避免 0xFF 被当成已启用配置而提前进入反转/刹车逻辑。
    eepromBuffer.rc_car_reverse = 0;
    ...
    eepromBuffer.active_brake_power = 0;
    uint32_t pole_pairs = eepromBuffer.motor_poles / 2;
    fixed_speed_mode_stage = 0;
    fixed_speed_mode_counter = 0;
    input_override = 48 * 10000;
    speedPid.error = 0;
    speedPid.integral = 0;
    target_e_com_time = pole_pairs > 0 ? (60000000UL / FIXED_SPEED_MODE_RPM / pole_pairs) : 0;
    input = 48;
#endif
