...
static const E1_Profile_t g_e1_profiles[] =
{
    { "Si",  "SI",  "R1", E1_P0_TARGET_R1_RPM, E1_P0_SI_R1_DSHOT  },
    { "Si",  "SI",  "R2", E1_P0_TARGET_R2_RPM, E1_P0_SI_R2_DSHOT  },
    { "Si",  "SI",  "R3", E1_P0_TARGET_R3_RPM, E1_P0_SI_R3_DSHOT  },
    { "GaN", "GAN", "R1", E1_P0_TARGET_R1_RPM, E1_P0_GAN_R1_DSHOT },
    { "GaN", "GAN", "R2", E1_P0_TARGET_R2_RPM, E1_P0_GAN_R2_DSHOT },
    { "GaN", "GAN", "R3", E1_P0_TARGET_R3_RPM, E1_P0_GAN_R3_DSHOT }
};

#define E1_PROFILE_COUNT                  ((uint8_t)(sizeof(g_e1_profiles) / sizeof(g_e1_profiles[0])))
...
typedef struct
{
    ...
    uint32_t fast_current_trip_start_tick;
...
static uint8_t E1_IsStopCommand(const char *text);
static void E1_ProcessStartCommand(void);
static void E1_ProcessStopCommand(void);
...
static const E1_Profile_t *E1_GetSelectedProfile(void);
static void E1_ApplyProfile(uint8_t profile_index);
static void E1_SendProfileLine(const char *event);
...
void E1_Test_Task(void)
{
    ...
    case STATE_DONE:
        if ((state_elapsed_ms >= E1_DONE_REARM_DELAY_MS) &&
            ((g_h1.safety_fault_latched == 0U) || (g_h1.manual_stop_requested != 0U)))
        {
            g_h1.start_command_received = 0U;
            g_h1.start_ack_pending = 0U;
            g_h1.start_armed_once = 0U;
            g_h1.manual_stop_requested = 0U;
            g_h1.safety_fault_latched = 0U;
            g_h1.current_trip_start_tick = 0U;
            g_h1.fast_current_trip_start_tick = 0U;
            g_h1.test_start_tick = now_ms;
            g_h1.last_csv_tick = now_ms;
            E1_Test_EnterState(STATE_WAIT_BT);
            E1_SendProfileLine("rearmed");
        }
        break;
...
static void E1_UpdateRampThrottle(uint32_t now_ms)
{
    ...
    ramp_start = g_h1.ramp_start_dshot;
    ramp_target = g_h1.ramp_target_dshot;
    ...
    hold_ms = E1_RAMP_START_HOLD_MS;
    if (E1_RAMP_MS <= 1U)
    {
        hold_ms = 0U;
    }
    else if (hold_ms >= E1_RAMP_MS)
    {
        hold_ms = E1_RAMP_MS - 1U;
    }
    ...
    ramp_duration_ms = E1_RAMP_MS;
    if (hold_ms > 0U)
    {
        ramp_elapsed_ms = elapsed_ms - hold_ms;
        ramp_duration_ms = E1_RAMP_MS - hold_ms;
    }
...
static void E1_UpdateCurrentDerived(E1_AdcProcessed_t *adc_data, uint32_t now_ms)
{
    ...
    adc_data->instant_current_A = instant_current_a;
...
static void E1_Safety_Task(uint32_t now_ms)
{
    ...
    if (E1_CURRENT_FAST_TRIP_A > 0.0f)
    {
        if (g_h1.adc.instant_current_A >= E1_CURRENT_FAST_TRIP_A)
        {
            if (g_h1.fast_current_trip_start_tick == 0U)
            {
                g_h1.fast_current_trip_start_tick = now_ms;
            }
            else if ((now_ms - g_h1.fast_current_trip_start_tick) >= E1_CURRENT_FAST_TRIP_HOLD_MS)
            {
                E1_TriggerSafetyFault("fast_overcurrent");
                return;
            }
        }
        else
...
{
    char line[224];
    char filtered_current_str[20];
    char instant_current_str[20];
    uint32_t t_ms;
    int len;
    ...
    E1_FormatFloat3(filtered_current_str, sizeof(filtered_current_str), g_h1.adc.current_A);
    E1_FormatFloat3(instant_current_str, sizeof(instant_current_str), g_h1.adc.instant_current_A);
    t_ms = g_h1.state_enter_tick - g_h1.test_start_tick;
    len = snprintf(line,
                   sizeof(line),
                   "# safety_fault,t_ms=%lu,reason=%s,current_A=%s,instant_current_A=%s,cycle=%u,action=stop\r\n",
                   (unsigned long)t_ms,
                   (reason != NULL) ? reason : "unknown",
                   filtered_current_str,
                   instant_current_str,
...
{
    const E1_Profile_t *profile = E1_GetSelectedProfile();
    char line[360];
    char v_i_sense_str[20];
    char v_vbat_adc_str[20];
    char current_a_str[20];
    char instant_current_a_str[20];
    char vbat_v_str[20];
    char power_w_str[20];
    ...
    E1_FormatFloat3(instant_current_a_str, sizeof(instant_current_a_str), adc_data->instant_current_A);
    ...
    len = snprintf(line,
                   sizeof(line),
                   "h1:%lu,%u,%u,%u,%u,%u,%s,%s,%s,%s,%s,%s,%s,%s,%u,%u,%s\r\n",
                   (unsigned long)t_ms,
...
static void E1_Test_EnterState(E1_TestState_t new_state)
{
    ...
    case STATE_RAMP:
        ...
        g_h1.ramp_start_tick = g_h1.state_enter_tick;
        g_h1.ramp_start_dshot = E1_GetRampStartDshot();
        g_h1.ramp_target_dshot = g_h1.run_cmd_dshot;
        set_throttle_command(g_h1.ramp_start_dshot);
        g_h1.last_dshot_send_tick = 0U;
        E1_Dshot_Service(g_h1.state_enter_tick);
        break;
...
static void E1_StartSession(uint32_t now_ms)
{
    ...
    E1_ApplyProfile(g_h1.profile_index);
    E1_SendStartAck();
    E1_SendProfileLine("start_profile");
    g_h1.start_armed_once = 0U;
    g_h1.header_sent = 0U;
...
static void E1_SendBootBannerOnce(void)
{
    static const char banner[] =
        "# boot,t_ms=0,fw=E1_DSHOT300_SAMERPM_PROFILES,uart=9600,protocol=firewater\r\n";
...
static void E1_SendConnectedBannerOnce(void)
{
    ...
    len = snprintf(line,
                   sizeof(line),
                   "# connected,t_ms=0,state=%s,prepare_ms=%lu,confirm_ms=%lu,cycles=%u,ramp_start=%u,ramp_hold_ms=%lu,ramp_ms=%lu,run_ms=%lu,rest_ms=%lu,profile=%u,board=%s,rpm_point=%s,target_rpm=%u,run_cmd_raw=%u,dshot_start=after_prepare\r\n",
                   ...
                   (unsigned long)E1_RAMP_START_HOLD_MS,
...
static void E1_SendWaitStartBannerOnce(void)
{
    static const char line[] =
        "# waiting,start_cmd=START,button=cycle_p0_profiles,action=prepare_ramp_then_3_auto_cycles\r\n";
...
static void E1_SendStartAck(void)
{
    ...
    len = snprintf(line,
                   sizeof(line),
                   "# ack,start_received,profile=%u,board=%s,rpm_point=%s,target_rpm=%u,run_cmd_raw=%u,action=enter_prepare_then_3_cycles\r\n",
                   (unsigned int)(g_h1.profile_index + 1U),
                   profile->board_name,
                   profile->rpm_point,
                   (unsigned int)profile->target_rpm,
                   (unsigned int)profile->dshot_cmd);
    if ((len > 0) && (E1_UartWrite(line) != 0U))
    {
        g_h1.start_ack_pending = 0U;
    }
}
...
static void E1_SendCsvHeaderOnce(void)
{
    static const char header[] =
        "# firewater_channels,ch0=t_ms,ch1=state,ch2=cmd_raw,ch3=dshot_cmd,ch4=adc_i_raw,ch5=adc_vbat_raw,ch6=v_i_sense,ch7=v_vbat_adc,ch8=current_A,ch9=vbat_V,ch10=power_W,ch11=zero_offset_V,ch12=active_zero_offset_V,ch13=delta_i_V,ch14=profile,ch15=target_rpm,ch16=instant_current_A\r\n";
...
static void E1_SendStateChangeLine(E1_TestState_t state)
{
    ...
    len = snprintf(line,
                   sizeof(line),
                   "# state_change,t_ms=%lu,state=%u,name=%s,cycle=%u,total_cycles=%u,profile=%u,board=%s,rpm_point=%s,target_rpm=%u,current_dshot_raw=%u,run_cmd_raw=%u\r\n",
                   (unsigned long)t_ms,
                   (unsigned int)state,
...
static void E1_HandleThrottleButtonPress(void)
{
    ...
    if (next_profile >= E1_PROFILE_COUNT)
    {
        next_profile = 0U;
    }

    E1_ApplyProfile(next_profile);
    E1_SendProfileLine("profile_set");
...
static const E1_Profile_t *E1_GetSelectedProfile(void)
{
    if (g_h1.profile_index >= E1_PROFILE_COUNT)
    {
        g_h1.profile_index = 0U;
    }

    return &g_e1_profiles[g_h1.profile_index];
}

static void E1_ApplyProfile(uint8_t profile_index)
{
    const E1_Profile_t *profile;

    if (profile_index >= E1_PROFILE_COUNT)
    {
        profile_index = 0U;
    }

    g_h1.profile_index = profile_index;
    profile = E1_GetSelectedProfile();
    g_h1.run_cmd_dshot = E1_ClampDshotThrottle(profile->dshot_cmd);
}
...
static void E1_SendProfileLine(const char *event)
{
    ...
    len = snprintf(line,
                   sizeof(line),
                   "# %s,profile=%u,total_profiles=%u,board=%s,rpm_point=%s,target_rpm=%u,run_cmd_raw=%u\r\n",
                   (event != NULL) ? event : "profile",
                   (unsigned int)(g_h1.profile_index + 1U),
                   (unsigned int)E1_PROFILE_COUNT,
                   profile->board_name,
                   profile->rpm_point,
                   (unsigned int)profile->target_rpm,
                   (unsigned int)g_h1.run_cmd_dshot);
...
static void E1_Oled_Update(uint32_t now_ms)
{
    ...
    if (g_h1.state == STATE_WAIT_BT)
    {
        (void)snprintf(line4, sizeof(line4), "%s", (g_h1.bt_confirmed != 0U) ? "WAIT START" : "WAIT BT");
    }
    else if (g_h1.state == STATE_PREPARE)
    {
...
static void E1_ProcessReceivedByte(uint8_t byte)
{
    ...
    if ((c == '\r') || (c == '\n'))
    {
        if (g_uart_cmd_index > 0U)
        {
            ...
            if ((g_h1.bt_confirmed != 0U) &&
                (E1_IsStopCommand(g_uart_cmd_buffer) != 0U))
            {
                E1_ProcessStopCommand();
            }
            else if ((g_h1.state == STATE_WAIT_BT) &&
                     (g_h1.bt_confirmed != 0U) &&
...
static void E1_ProcessStopCommand(void)
{
    char line[96];
    uint32_t now_ms = HAL_GetTick();
    uint32_t t_ms = now_ms - g_h1.test_start_tick;
    int len;
    ...
    if (g_h1.safety_fault_latched == 0U)
    {
        g_h1.manual_stop_requested = 1U;
    }
    g_h1.current_trip_start_tick = 0U;
    g_h1.fast_current_trip_start_tick = 0U;

    if ((g_h1.state == STATE_PREPARE) || (g_h1.state == STATE_RAMP) ||
        (g_h1.state == STATE_RUN) || (g_h1.state == STATE_STOP))
    {
        g_h1.state = STATE_DONE;
        g_h1.state_enter_tick = now_ms;
    ...
    len = snprintf(line,
                   sizeof(line),
                   "# user_stop,t_ms=%lu,action=forced_dshot_stop\r\n",
                   (unsigned long)t_ms);
    if (len > 0)
    {
        E1_UartWrite(line);
